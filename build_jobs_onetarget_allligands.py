...
def build_string_list(final_list, ligand_path, name_list):
    command_string_list = []
    global next_job_counter
    for a, lig_group in enumerate(final_list):
        string = "python3 " + software_path + "FastAID_Py/main.py " + ligand_path + " " + receptor + " " + binding_site + ' '
...
if __name__ == "__main__":
    ...
    #END PREP
    receptor = base_path + main_receptor + "/" + find_file(".inp.pdb", base_path + main_receptor + "/")  # .inp.pdb
    binding_site = base_path + main_receptor + "/" + "getcleft/" + find_file("_sph_1.pdb",
                                                                             base_path +
                                                                             main_receptor + "/" + "getcleft/")
    ...
    for a, dir in enumerate(dirs):
        ...
        active_string_list = build_string_list(active_final_list, active_ligand_path, active_final_name_list)
        decoy_string_list = build_string_list(decoy_final_list, decoy_ligand_path, decoy_final_name_list)
